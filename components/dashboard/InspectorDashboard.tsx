...
import { ClipboardList, Map, BarChart2, MessageSquare, ChevronRight } from 'lucide-react';
import { RiskCategory } from '../../types';

const InspectorDashboard: React.FC = () => {
    const { user, reports, getLocationById, getZoneByLocationId, getFormById, theme } = useContext(AppContext);
    const { t } = useI18n();
    ...
    const kpiData = useMemo(() => {
        ...
        const visitedZones = new Set(filteredReports.map(r => getZoneByLocationId(r.locationId)?.id)).size;
        const totalScore = filteredReports.reduce((acc, report) => acc + calculateScore(report), 0);
        const averageScore = totalReports > 0 ? (totalScore / totalReports).toFixed(1) : 0;
        return { totalReports, visitedZones, averageScore };
    }, [filteredReports]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <KpiCard title={t('totalReports')} value={kpiData.totalReports} icon={<ClipboardList className="text-brand-blue"/>} />
                <KpiCard title={t('visitedZones')} value={kpiData.visitedZones} icon={<Map className="text-brand-teal"/>} />
                <KpiCard title={t('averageScore')} value={kpiData.averageScore} unit="%" icon={<BarChart2 className="text-brand-orange"/>} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                            ...
                            <YAxis tick={{ fill: tickColor }} unit="%" />
                ...
                <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm">
                    <h3 className="font-bold text-brand-blue-dark dark:text-brand-green mb-4">{t('riskCategoryDistribution')}</h3>
                    <ResponsiveContainer width="100%" height={300}>
                         <PieChart>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                {filteredReports.slice().reverse().slice(0, 5).map(report => {
                                    ...
                                    return (
                                        <tr key={report.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">{location?.name}</td>
                ...
                <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm">
                    <h3 className="font-bold text-brand-blue-dark dark:text-brand-green mb-4">{t('supervisorFeedback')}</h3>
                                        ...
                                        <div className="flex items-start space-x-3 rtl:space-x-reverse">
                                            ...
                                            <div>
                                                <p className="font-semibold text-sm text-gray-800 dark:text-gray-200 group-hover:text-brand-blue">
                                                    {getLocationById(report.locationId)?.name}
