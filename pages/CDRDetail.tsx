...
import { CDR, CDRStatus, CDRIncidentType, CDRManagerDecision, UserRole } from '../types';
import { LOCATIONS, SERVICE_TYPES, MANPOWER_DISCREPANCY_OPTIONS, MATERIAL_DISCREPANCY_OPTIONS, EQUIPMENT_DISCREPANCY_OPTIONS, ON_SPOT_ACTION_OPTIONS, ACTION_PLAN_OPTIONS } from '../constants';
import { Save, Send, ShieldCheck, Printer, Upload, CheckCircle, Hospital, X } from 'lucide-react';

const CDRDetail: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const { user, getCDRById, addCDR, updateCDR } = useContext(AppContext);
    const { t } = useI18n();
                ...
                {isEditableOnScreen ? (
                    ...
                    <Section title="basicIncidentInformation">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <SelectField label="wardLocation" name="location" value={cdr.location} onChange={handleInputChange} disabled={!isEmployeeEditable} options={LOCATIONS.map(l => l.name)} />
                            <InputField label="department" name="department" value={cdr.department} onChange={handleInputChange} disabled={!isEmployeeEditable} />
                            <SelectField label="typeOfIncident" name="incidentType" value={cdr.incidentType} onChange={handleInputChange} disabled={!isEmployeeEditable} options={Object.values(CDRIncidentType)} />
                        </div>
                    </Section>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-4">
                                    {Object.values(CDRManagerDecision).map(decision => (
                                        <label key={decision} className="flex items-center space-x-2">
                                            <input type="radio" name="managerDecision" value={decision} checked={cdr.managerDecision === decision} onChange={handleInputChange} />
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm cdr-print-view">
            ...
            <div className="cdr-print-grid">
                <ReadOnlySection title="basicIncidentInformation">
                    ...
                    <p><strong>{t('location')}:</strong> {cdr.location}</p>
...
const SelectField: React.FC<{label: string, name: string, value: string, onChange: any, disabled: boolean, options: string[]}> = ({label, name, value, onChange, disabled, options}) => {
    const {t} = useI18n();
    return(
    <div>
        <label className="block text-sm font-medium mb-1">{t(label)}</label>
